...
<td><p>VT_BOOL</p></td>
<td><p>Optional</p></td>
<td><p>If this value is set to TRUE, the ambient light sensor cannot currently return any valid sample. This value may be set to TRUE when the sensor field of view is obstructed (such as when an object, or the user hand is in front of the sensor). This value should be set to FALSE when the ambient light sensor is able to accurately. Proper hardware design should try to minimize the time and scenarios requiring this value to be set to TRUE as such scenario prevents the system from properly controlling brightness. On an ideal system, this value would always be set to FALSE.</p></td>
</tr>
</tbody>
</table>
